...
class PacGame(Widget):
    ...
    def __init__(self, **kwargs):
        ...
        self.build_level()
        self.status.text = "COMEÇAR O GAME?"
    ...
    def death(self):
        ...
        if self.pac.end_angle > 90 + self.pac.rotation:  # Death animation.
            ...
            self.status2.text = "Pressione Enter Para Reiniciar o JOGO!"
    ...
    def respawn_player(self, restart=False):
        ...
        self.redraw(self.status)
        self.redraw()
        self.status.text = "DENOVO,FRACO!"
        self.ready_check = True

    def win(self):
        # Display win status and wait for input to reset game.
        self.status.text = "PARABÉNS!! Hehe"
        self.status2.text = "APERTE ENTER!"
        self._check = True
        self.redraw(self.status)
        self.redraw(self.status2)
